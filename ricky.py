...
@click.argument("output", type=click.Path())
def embed(input, embedding, output):
    main_image = Image.open(input)
    embedding_image = Image.open(embedding)
    w, h = embedding_image.size
    main_image = main_image.resize((w * 2, h * 2))
    main_image = main_image.point(lambda data: data * 0.8)
    for x in range(w):
        for y in range(h):
            main_image.putpixel((x*2,y*2), embedding_image.getpixel((x,y)))
    main_image.info = {
